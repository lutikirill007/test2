...
def print_data():  # вывод в терминал
    m = max([len(i) for i in columns])  # максимальная длина слова  (lastname)
    for i in columns:
        print(str(i).ljust(m + 1, ' '),
              end='')  # ljust добавляет пробелы к названиям колонок и данныым чтобы вывод был читаемый
    print()
    for line in data:
        for i in line:
            print(str(i).ljust(m + 1, ' '),
...
def read_from_file(filename):  # чтение из файла и запись в переменные colums и data
...
columns, data = read_from_file(
    'C:\\Users\\Дмитрий\\Desktop\\kurs_python\\modul_2\\Step3\\data.csv')  # вызов функции для записи в перменные
